...
class DynamicSelfCompositionPDR_qe:
    ...
    def solve(self,num_new_preds):
        ...
        #print('solve,2')
        trace = self.find_cex()
        #print('trace in solve method',trace)
        if not( trace == None):
            #print('solve,3')
            self.trc = trace
        #print('solve,4')
        ...
        while trace is not None:
            print('Exploring next composition.. \n\n')
            #print('solve,5')
            # if time.time() > timeout:
            #     return "Timeout reached",self.smt_queries_count
            #if self.print_log:
            print("Abstract Counter-example trace for current composition:\n" + self.print_trace(trace))
           #batch 5 : cex_trace_states
            ...
            if not fixed:
                ...
                print('Is the trace spurious? ',spur)
                #print('solve,14')
                ...
                if spur == True :
                    #print('solve,19')
                    new_pred_smt_format = []
                    new_pred_smt_format.append(self.pred_disc(c_src,c_tgt,a_src,a_tgt,tr_formula))
                    #print('solve,20')
    ...
    def pred_disc_refine_bad(self,a_tgt):
        ...
        cmd = 'python3 original_bad.py > original.txt'
        os.system(cmd)
        print('pred_disc,11')
        print('Finished dumping orihinal to py file, now reading from txt....')
        #read the file
        f_txt = open('original.txt','r')
   ...
   '''
    ...
    def check_spur(self,trace):
        ...
        if s_find_next.check() == z3.unsat :
            ...
            print('--------------------------------------------------')
            print('Initial concrete state : ',values_dict)
            print('\n\n')
            formula = []
    ...
    def pred_disc(self,c_src,c_tgt,a_src,a_tgt,tr):
            print('Discovering predicate from a_src, a_tgt, c_src, c_tgt to eliminate spurious trace')
            print('----------------------pred_disc-------------------')
           ...
           # x, y, z, w = Ints('x y z w')
            ...
            #add queries to sl file
            new_pred_smt_format = self.dump_to_smt_file(c_src,c_tgt,str_prefix_a_src,str_prefix_a_tgt,vars_in_c_src,vars_in_c_src_p,tr_wo_let)
            #print('new_pred_smt_format',new_pred_smt_format)
            '''
    ...
    def add_new_predicate_smt(self,smt_predicate,c_src,refine_type):
            #add the predicate to the original smt2 file
            #print('typo00')


            #print('new_predicate at start of add_new_predicate',smt_predicate)
            ...
            if smt_predicate.__contains__('let'):
                ...
                #print('New predicate without let\n'+predicate_wo_let_maybe_with_not)
            else :
                predicate_wo_let_maybe_with_not = smt_predicate

            #print('reached the end')
            '''
            if not(final_predicate[0:5].find('not') == -1):
                final_predicate = (final_predicate[5:])[:-1]
            else:
               final_predicate = '( not '+final_predicate+')'
            '''

            '''
            #assumes the output is of the form (not( ...)) or ().
            if new_predicate_without_let[0:4].__contains__('(not'):
                final_predicate = new_predicate_without_let[5:][:-1]

            else:
                final_predicate = new_predicate
            print('final predicate is ',final_predicate)
            '''
            ...
            #print('last version of predicate',pred_body)
   ...
   '''         '''
    ...
    def dump_to_smt_file(self,c_src,c_tgt,a_src,a_tgt,vars_in_c_src,vars_in_c_src_p,tr):
            ...
            vars_in_c_src_U_str = vars_in_c_src_str.replace('_','U')
            ...
            vars_in_c_src_U_str_for_p = vars_in_c_src_str_for_p.replace('_','U')
            vars_in_c_src_strp = ''
            #print('\n\n yay %s \n\n' %(a_src))
            for v in vars_in_c_src_p:
                vars_in_c_src_strp = vars_in_c_src_strp + ' '+v
            #print('hereee',vars_in_c_src_strp)
            vars_in_c_src_U_strp = vars_in_c_src_strp.replace('_','U')
            ...
            for x in vars_in_c_src:
                trans_fun = trans_fun + '(declare-fun '+str(x).replace('_','U')+'()  '+ str(vars_in_c_src[x])+' )\n'

            for x in vars_in_c_src_p:
                trans_fun = trans_fun + '(declare-fun '+str(x).replace('_','U')+'()  '+ str(vars_in_c_src_p[x])+' )\n'
            trans_fun = trans_fun + '\n'
            ...
            for x in vars_in_c_src:
                one = one + '('+str(x).replace('_','pU')+' '+ str(vars_in_c_src[x])+' )'

            one = one +')(and (aUsrc '+vars_in_c_src_U_str+') (trans '+vars_in_c_src_U_str + vars_in_c_src_U_strp +') (not (not (aUtgt '+vars_in_c_src_U_strp+'))))))\n'
            f.write(one)
            two = '(assert (aUsrc '+vars_in_c_src_U_str+'))'
            ...
            for line_no in range(len(datafile)):
                #print('line_no',line_no,datafile[line_no],type(datafile[line_no]));
                if datafile[line_no].__contains__('After'):
                    #print("Contains")
                    new_predicate = '\n'.join(datafile[line_no+1:])
                    break
            print('New predicate is ',new_predicate)
            ...
            #print('new_predicate is '+str(new_predicate)+'\n\n')
            if len(new_predicate) > 2:
                rule = self.add_new_predicate_smt(new_predicate,c_src,'synth')
            else:
                exit()
                rule = 'unknown'
            return rule
