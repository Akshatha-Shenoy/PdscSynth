...
class DynamicSelfCompositionPDR_msat_qe:
    def __init__(self, program_path, force_predicate_abstraction, is_comparator, explicit_conposition_function,
                 folder_name, filename_sign, method_name=None, concrete_array_vars=False, bmc=False, print_log=False, prop=None):
        ...
        self.i_fname = self.filename_sign
            ...
            if not fixed:
                ...
                if spur == True:
                    if self.num_new_preds < 20:
                        ...
                        if new_pred_smt_format == []:
                            solver = DynamicSelfCompositionPDR_qe(self.program_path, force_predicate_abstraction=True,
                                                                           is_comparator=False, method_name="compare",
                                                                           bmc="True",
                            ...
                            msg, smt_count, num_preds, restart, new_predicates = solver.solve(self.num_new_preds)
                            print(msg, smt_count, num_preds, restart, new_predicates )
                            return "Failed to find semantic self-composition. Either the property is violated or not enough predicates were supplied.", smt_count, num_preds, restart, new_predicates
    ...
    def write_to_msat(self):
        print('Writing an smt file to be run using mathsat')
        # declare variables
        # create an msat file
        ...
        f_msat.write('(set-option :produce-models true)\n\n')
        for key, value in self.dynamic_program.var_string_dict.items():
            key_n_p = str(key)
            for index in (range(len(self.cex_trace_states))):
        ...
        f_msat.close()
        # run it
        msat_cmd = './mathsat -model ' + str(self.i_folder_path) + str(self.i_fname) + '_msat' + '.smt2 > ' + str(
    ...
    def get_interpolants_from_file(self, solver_name):
        print('Checking output from mathsat 5')
        if solver_name == 'smtInt':
            ...
            #print('Read interpolants')
            #print('num_of_lines ', num_of_lines)
            #print(lines[0])
            exit(0)
            if lines[0].find('unsat') == -1:
                print(lines[1:-1])
                print('Program unsafe due to above concrete counterexample trace.')
                exit(0)
                return ['-1']
            f_smtInt.close()
